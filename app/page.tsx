    ...
    <div className="min-h-screen bg-[#FFFBF7] text-zinc-900 antialiased selection:bg-[#E8A0BF]/30">
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 mt-10 sm:mt-12 grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-[28px] border border-[#FAD9E3] bg-gradient-to-br from-white to-[#FFF1F5] p-6 sm:p-8 flex flex-col">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-600">Trả lời 3 câu hỏi nhanh, hệ thống demo sẽ gợi ý routine phù hợp.</p>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 mt-10 sm:mt-12">
        <div className="rounded-[28px] bg-white border border-zinc-100 p-6 sm:p-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between shadow-sm">
          <div className="max-w-[520px]">
            ...
            <p className="mt-1.5 text-sm leading-6 text-zinc-600">Nhập email để nhận mã DEMO20 (-20% đơn đầu). Demo không gửi email thật.</p>
          </div>
          <form onSubmit={e=>{e.preventDefault(); alert("Demo: Đã lưu email! Mã của bạn: DEMO20")}} className="flex w-full max-w-[420px] items-center gap-2">
        ...
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-500">Website demo bán mỹ phẩm responsive — không kinh doanh thật. Dữ liệu giả lập để trình bày giao diện.</p>
            </div>
            {[
              ["Cửa hàng", ["Skincare","Makeup","Body & Hair","Beauty Box"]],
              ["Hỗ trợ", ["Vận chuyển","Đổi trả 14 ngày","FAQ","Liên hệ"]],
              ["Thanh toán demo", ["VNPay (demo)","MoMo (demo)","ZaloPay (demo)","* Không trừ tiền thật"]],
            ].map(([title, links])=> (
          ...
          <div className="absolute right-0 top-0 flex h-full w-full max-w-[420px] flex-col bg-white shadow-2xl">
            ...
            <div className="flex-1 overflow-auto p-6">
              {cart.length===0 ? (
                <div className="py-16 text-center">
                  ...
                  <div className="text-sm text-zinc-500">Thêm sản phẩm để xem drawer responsive này</div>
            ...
            {cart.length>0 && (
              <div className="border-t p-6 space-y-3 bg-white">
                ...
                <div className="flex justify-center gap-2 text-[11px] text-zinc-500"><span>VNPay</span>•<span>MoMo</span>•<span>ZaloPay</span></div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          <div className="relative w-full max-w-[520px] max-h-[92vh] overflow-auto rounded-t-[28px] sm:rounded-[28px] bg-white shadow-2xl">
            ...
            <div className="p-6">
              {!quizAns ? (
                <>
                  <p className="text-sm text-zinc-600">Chọn mô tả gần nhất với da bạn sau khi rửa mặt 30 phút:</p>
                  <div className="mt-5 grid gap-3">
                    {[
                      ["Da dầu", "Bóng dầu nhiều ở trán, mũi, cằm"],
                      ["Da khô", "Cảm giác căng, có vảy mịn"],
                      ["Da hỗn hợp", "Dầu vùng T, khô ở má"],
                      ["Da nhạy cảm", "Dễ đỏ, châm chích"],
                    ].map(([label, desc])=> (
                ...
                <div className="text-center py-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-zinc-600">Gợi ý: Với {quizAns.toLowerCase()}, bạn hợp với {quizAns==="Da dầu" ? "Serum Vitamin C + Mặt nạ đất sét" : "Cloud Cream + Sữa tắm Peony"}. Xem gợi ý ở lưới sản phẩm phía trên.</p>
                  <div className="mt-6 flex gap-3 justify-center">
                    <button onClick={()=>setQuizAns(null)} className="rounded-full border px-6 py-3 text-sm font-semibold">Làm lại</button>
                    <button onClick={()=>{setShowQuiz(false); document.getElementById("sanpham")?.scrollIntoView({behavior:"smooth"})}} className="rounded-full bg-zinc-900 px-6 py-3 text-sm font-semibold text-white">Xem sản phẩm</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-6 sm:p-8 flex flex-col">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-2 text-sm"><span className="font-semibold">★ {selected.rating}</span><span className="text-zinc-500">({selected.reviews} đánh giá)</span></div>
                </div>
              ...
              <div className="mt-5 space-y-3 text-sm leading-6 text-zinc-600 border-t pt-5">
                <p><b className="text-zinc-900">Công dụng:</b> Dưỡng sáng, cấp ẩm, đều màu da. Thuần chay, không thử nghiệm trên động vật.</p>
                <p><b className="text-zinc-900">Thành phần:</b> Vitamin C, Niacinamide, Hyaluronic Acid (demo).</p>
              </div>
      ...
      <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-30 hidden sm:flex items-center gap-2 rounded-full bg-zinc-900 px-4 py-2 text-xs font-medium text-white shadow-xl">
        <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" /> Responsive 320px → 1280px+
      </div>
    </div>
  );
}
